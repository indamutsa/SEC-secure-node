  ...
  router.put('/:itemId', async (req, res, next) => {
    try {
      const result = await TodolistService.updateItem(
        req.user.id, // Integrating object level authorization by adding the user id to the updateItem method
        req.params.itemId,
        req.body
      );
  ...
  router.delete('/:itemId', async (req, res, next) => {
    try {
      const result = await TodolistService.deleteItem(
        req.user.id, // Integrating object level authorization by adding the user id to the deleteItem method
        req.params.itemId
      );
      return res.json({ result });
    } catch (err) {
