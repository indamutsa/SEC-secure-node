    ...
    async (req, res, next) => {
      try {
        req.session.messages.push({
          text: 'You have been successfully authenticated with Github!',
          type: 'success',
        });
  ...
  router.get('/complete', async (req, res, next) => {
    try {
      if (!req.session.tempOAuthProfile) {
        req.session.messages.push({
          text: 'Failed to authenticate with GitHub!',
          type: 'danger',
        });
      ...
      if (req.user) {
        ...
        await user.save();
        req.session.messages.push({
          text: 'You have been successfully authenticated using GITHUB!',
          type: 'success',
        });
